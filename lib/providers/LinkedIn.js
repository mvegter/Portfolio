    ...
    switch (key) {
      case 'positions':
        for (const position of data.positions) {
          ...
          if (position.date) {
            position.date = position.date.replace('heden', 'current');
          }
        }
        break;
      default:
...
const scrapeProfile = async (username) => {
  ...
  try {
    ...
    logger.warn('LinkedIn | Could not crawl proflie due to: %s', error);
    if (fs.existsSync(cacheName)) {
      logger.info('LinkedIn | Failing over to local cache');
